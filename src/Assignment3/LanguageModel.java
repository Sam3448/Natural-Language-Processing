 ...
 */
abstract class LanguageModel implements java.io.Serializable {
    ...
    public void train(final String trainFile) throws IOException {
        System.err.format("Training from corpus %s\n", trainFile);
        ...
        System.err.format("Finished training on %d tokens\n", tokens.get(""));
        trained = true;
    }
    ...
    static final int numTokens(String trainFile) throws IOException {
        BufferedReader reader = openCorpus(trainFile);
        int count = 0;
        String line;
        while ((line = reader.readLine()) != null && line.length() != 0) {
    ...
    public static LanguageModel load(String fname) {
       try {
    ...
    public static void save(String fname, LanguageModel lm) {
      try {
